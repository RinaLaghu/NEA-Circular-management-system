...
def seed_data() -> None:
    ...
    try:
        ...
        directorate_map = {}
        for letter in sorted({d["directorate"] for d in departments}):
            directorate = Directorate(name=letter)
            db.add(directorate)
            directorate_map[letter] = directorate

        db.commit()
        for directorate in directorate_map.values():
