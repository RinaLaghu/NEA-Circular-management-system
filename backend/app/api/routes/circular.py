...
def get_circular_stats(
    ...
    if not current_dept:
        ...
        archived = db.query(Circular).filter(
...
def delete_draft_circular(circular_id: int, db: Session = Depends(get_db)):
    circular = db.query(Circular).filter(Circular.id == circular_id).first()
...
def send_circular(
    ...
    sender = db.query(Department).filter(
        Department.id == circular.sender_department_id
    ).first()

    receiver = db.query(Department).filter(
        Department.id == circular.receiver_department_id
    ).first()
